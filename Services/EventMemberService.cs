...
namespace com.petronas.myevents.api.Services
{
    public class EventMemberService : IEventMemberService
    {
        ...
        public EventMemberService(IEventRepository eventRepository)
        {
        ...
        public EventMember CheckExistence(string eventId, string userId)
        {
            ...
            return _eventRepository.GetAll(x => !x.IsDeleted && x.Id == eventId, feedOptions).FirstOrDefault()
                ?.Members.Where(x => !x.IsDeleted && x.UserId == userId).FirstOrDefault();
        }
        ...
        public async Task<bool> Join(string eventId, string userId)
        {
            ...
            {
                var oldMember = _event.Members.FirstOrDefault(x => !x.IsDeleted && x.UserId == userId);
