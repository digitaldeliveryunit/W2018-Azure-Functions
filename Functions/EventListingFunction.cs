    ...
    public static class EventListingFunction
    {
        ...
        public static IActionResult Run(
            [HttpTrigger(
                ...
                RequestMethods.Get,
                Route = "Events/Listing/{*listType}")]HttpRequest request,
                string listType,
            ILogger log,
                ...
                switch (request.Method)
                {
                    case RequestMethods.Get:
                        var query = new RouteRequest(){
                            ...
                            ContinuationKey = !string.IsNullOrEmpty(request.Query["continuationKey"]) ? EncryptionService.Decrypt(request.Query["continuationKey"]) : string.Empty,
                        ...
                        switch(listType){
                            case QueryConstants.EVENT_FEATURED: 
                                return new OkObjectResult(eventService.GetFeaturedEvents(query.ContinuationKey, query.Take, DefaultValue.UserId));
                            case QueryConstants.EVENT_UPCOMING: 
                                return new OkObjectResult(eventService.GetUpcomingEvents(query.ContinuationKey, query.Take, DefaultValue.UserId));
                            case QueryConstants.EVENT_UPCOMING_ALL: 
                                return new OkObjectResult(eventService.GetUpcomingAllEvents(query.ContinuationKey, query.Take, DefaultValue.UserId));
                            case QueryConstants.EVENT_PAST: 
                                return new OkObjectResult(eventService.GetPastEvents(query.ContinuationKey, query.Take, DefaultValue.UserId));
                            case QueryConstants.EVENT_SEARCH:
                                return new OkObjectResult(eventService.Search(query.SearchKey, query.ContinuationKey, query.Take, DefaultValue.UserId));
